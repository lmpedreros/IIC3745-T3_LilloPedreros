...
class ShoppingCartControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    sign_in @user_admin
    @product1 = Product.create!(nombre: 'Product1', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product2 = Product.create!(nombre: 'Product2', precio: 2000, stock: 5, user_id: @user_admin.id,
                                categories: 'Accesorio tecnologico')
    @product3 = Product.create!(nombre: 'Product3', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product4 = Product.create!(nombre: 'Product4', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product5 = Product.create!(nombre: 'Product5', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product6 = Product.create!(nombre: 'Product6', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product7 = Product.create!(nombre: 'Product7', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product8 = Product.create!(nombre: 'Product8', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
    @product9 = Product.create!(nombre: 'Product9', precio: 1000, stock: 200, user_id: @user_admin.id,
                                categories: 'Cancha')
  ...
  test 'should not insert product with insufficient stock' do
    ...
    assert_redirected_to root_path
    follow_redirect!
    assert_match "El producto '#{@product2.nombre}' no tiene suficiente stock para agregarlo al carro de compras.",
                 flash[:alert]
  end
  ...
  test 'should not insert more than 8 products in cart' do
    @shopping_cart = ShoppingCart.create!(user: @user_admin,
                                          products: { @product1.id => 1,
                                                      @product2.id => 1,
                                                      @product3.id => 1,
                                                      @product4.id => 1,
                                                      @product5.id => 1,
    ...
    assert_redirected_to root_path
    follow_redirect!
    assert_match 'Has alcanzado el máximo de productos en el carro de compras (8). ' \
  ...
  test 'shoukd not complete purchase if stock is not enough' do
    ...
    assert_redirected_to '/carro'
    follow_redirect!
    assert_match "Compra cancelada: El producto '#{@product2.nombre}' no tiene suficiente stock " \
                 'para realizar la compra. Por favor, elimina el producto del carro de compras o reduce ' \
                 'la cantidad.',
                 flash[:alert]
  end
