...
describe('Navegación a la creación de usuario', () => {
    it('Debería poder crear un usuario (No admin) NUEVO y volver a la página de inicio', () => {
...
describe('Navegación a la creación de usuario (Admin)', () => {
    it('Debería poder crear un usuario (Admin) NUEVO y volver a la página de inicio', () => {
        cy.visit('http://localhost:5017'); // cy.visit es para visitar la página... o algo así
        ...
        cy.fixture('seedData').then((data) => {
            ...
            cy.get('input[name="user[password]"]').type(user.password);
            cy.get('input[name="user[password_confirmation]"]').type(user.password);
            cy.get('input[name="user[role]"]').type(user.role);  // Esta línea también podría ser simplemente cy.get('input[name="user[role]"]').type("admin");
            cy.get('input[name="commit"]').click();
        });
        cy.url().should('not.include', '/register');
